...
var PhotoNav = {
	...
	init: function(id, width, width2, height, height2, smoothing, smoothingLevel, debugging) {
		...
		PhotoNav.containerHeight = height;
		PhotoNav.container = obj;
		PhotoNav.photo = document.getElementById('lightboxImage');
		
		PhotoNav.photoWidth = width2;
		PhotoNav.photo.style.width = width2 + "px";
		PhotoNav.photoHeight = height2;
		PhotoNav.photo.style.height = height2 + "px";
	},
	...
	getMouseXY: function (e) {
		...
		if (PhotoNav.tempY > containerY && PhotoNav.tempY < (containerY + containerH)) {			
			...
			var percX = (100 / (PhotoNav.containerWidth / (PhotoNav.tempX - PhotoNav.findPosX(PhotoNav.container))));
			var percY = (100 / (PhotoNav.containerHeight / (PhotoNav.tempY - PhotoNav.findPosY(PhotoNav.container))));
			...
			if (percX < 0) {
				...
				PhotoNav.addStatus("Mouse X: " + PhotoNav.tempX + " / " + PhotoNav.winW + " = " + percX + "%");
				
				PhotoNav.posPicture(percX, percY);
			}
		} else {
			PhotoNav.addStatus("Not in element.");
		}
	...
	posPicture: function(x, y) {
		...
		var full = PhotoNav.photoHeight;
		full = full - PhotoNav.containerHeight;
		var curY = full * (y / 100);
		...
		if (PhotoNav.smoothing == true) {
			...
			PhotoNav.currentEffect = new Effect.Morph(PhotoNav.photo, {
			  style: 'margin-left:-' + curX + 'px; margin-top:-' + curY + 'px;',
			  duration: PhotoNav.smoothingLevel
			});	
		} else {
			PhotoNav.photo.style.marginLeft = "-" + curX + "px";
			PhotoNav.photo.style.marginTop = "-" + curY + "px";
		}
